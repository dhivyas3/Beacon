...
## 2026-09-24 Chromium always disables `/dev/shm`; `--no-sandbox` is opt-in

Preparing a Railway deploy surfaced a latent gap: the worker's Chromium launch has never passed any flags in production, and was only working because Docker Compose gives the worker container `shm_size: 1gb`, well above Docker's 64 MB default. Most PaaS containers, Railway included, do not let a service raise that limit, so Chromium would crash the moment a page did anything memory-heavy. `--disable-dev-shm-usage` (write those temp files to `/tmp` instead) is now always on (`chromiumArgs` in `apps/worker/src/scan/browser.ts`), everywhere, including development, since it has no downside. `--no-sandbox` is different: some container runtimes refuse to let Chromium create its own sandbox at all, but the flag is a real isolation trade-off against a page that exploits a Chromium bug, so it stays off unless `CHROMIUM_NO_SANDBOX=true` is set, which only the operator of a specific deploy should decide.

## 2026-09-24 Railway: one service for api and worker, because volumes are not shared

A Railway Volume attaches to exactly one service. The api and the worker both need the same screenshot files on disk (the worker writes them, the api serves and exports them), which Docker Compose gets for free from one named volume mounted into two containers — Railway has no equivalent. Rather than add a network filesystem or build the S3 backend `@beacon/storage`'s own interface comment already anticipates (real work, not needed to get a first deploy running), `deploy/railway/api-worker.Dockerfile` runs both processes in one container on one volume: migrate, seed if the admin variables are set, then the worker in the background and the api in the foreground, each still exiting the whole container if the other dies (see `run-api-and-worker.sh`) so Railway's restart policy is the same safety net it would be for two separate crash-looping services. The cost is that the pair cannot be scaled independently, and the service must never run more than one replica, or two volumes would silently split the screenshots between them. `web` (nginx, no volume) is unaffected and scales freely.

## 2026-09-24 Railway's web image renders its nginx config from a template at container start

`PORT` is assigned per deploy and is not known at build time, and the address of the `api-worker` service (`API_INTERNAL_URL`) is only known once that service exists. `deploy/railway/nginx.conf.template` is rendered by `envsubst` in `docker-entrypoint.sh`, restricted to exactly those two variable names (`envsubst '${PORT} ${API_INTERNAL_URL}'`) so nginx's own `$`-prefixed variables in the same file (`$http_host`, `$uri`, ...) are left alone instead of being substituted into nothing. This is the same proxy, including the unbuffered `/api/` location for the event stream, as `deploy/nginx.conf` for Docker Compose; that file is untouched.

## 2026-09-24 The Docker Compose api image never actually auto-seeded the admin

`docs/DEPLOYMENT.md` claimed the API "creates the first administrator from `SEED_ADMIN_*`" on start. It never did: the root `Dockerfile`'s `api` target only ever ran `prisma migrate deploy`, and nothing in `apps/api` calls the seed script. Found while writing the Railway equivalent. The Compose doc now says to run `docker compose run --rm api pnpm --filter @beacon/db seed` once, matching what actually happens; the root `Dockerfile` itself is left as-is, since changing what every existing Compose deploy runs on boot was not asked for. The new Railway image does seed automatically (guarded on both `SEED_ADMIN_*` variables being set, so it is a no-op rather than a crash when they are not), because it is new and nothing depends on its current boot behaviour yet.

## 2026-09-24 Railway's build validator rejects a cache mount `id` with no prefix of its own

The first real Railway deploy attempt failed before the build even started: `dockerfile invalid: flag '--mount=type=cache,id=pnpm-store,target=/pnpm/store' is missing the cacheKey prefix from its id`. This is a Railway-specific check in front of BuildKit, not a real Dockerfile error — `id=pnpm-store` is ordinary, portable syntax that Docker Desktop, GitHub Actions and every other builder accept without complaint, including the root `Dockerfile`'s own copy of this line, which is untouched because Railway never sees it. Rather than guess at whatever prefix format Railway wants, both Railway Dockerfiles drop the explicit `id` entirely (`--mount=type=cache,target=/pnpm/store`): BuildKit derives one from the mount target on its own, which is exactly as portable and sidesteps Railway's requirement instead of trying to satisfy it.
